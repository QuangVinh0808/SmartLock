...
# ===================== CONFIG =====================
MODEL_PATH = "models/facenet.tflite"   # model lấy từ repo pillarpond
USER_DB_DIR = "users"

# Cosine similarity threshold (tune theo data nhà bạn)
# FaceNet + prewhiten: thường 0.45~0.70 tùy chất lượng ảnh/cam
COSINE_THRESHOLD = 0.55

REQUIRED_TIME = 5          # giữ mặt đủ N giây mới unlock
SEND_UNLOCK_ONCE = True    # chỉ gửi '1' 1 lần khi đủ thời gian, rồi tự về 0 khi mất mặt
...
# Model from repo: input 160x160, embedding 512 (we still read from model to be safe)
in_shape = input_details[0]["shape"]  # e.g. [1,160,160,3]
out_shape = output_details[0]["shape"]  # e.g. [1,512]
...
if IN_DTYPE != np.float32:
    print("⚠️ Model này expected float32 theo repo. Nếu dtype khác, bạn báo mình để chỉnh lại.")

# ===================== PREPROCESS (FaceNet prewhiten) =====================
def prewhiten(img_rgb_float_0_255: np.ndarray) -> np.ndarray:
    """
    img_rgb_float_0_255: shape (H,W,3), float32, range [0..255]
    return: shape (H,W,3), float32, prewhitened
    Repo dùng: x = (x - mean) / max(std, 1/sqrt(N))  :contentReference[oaicite:1]{index=1}
    """
...
def get_face_embedding(face_bgr: np.ndarray) -> np.ndarray | None:
    ...
    # FaceNet prewhiten (đúng repo)
    face_input = prewhiten(face_rgb)

    # add batch
    face_input = np.expand_dims(face_input, axis=0).astype(np.float32)  # [1,H,W,3]

    interpreter.set_tensor(input_details[0]["index"], face_input)
    interpreter.invoke()
    emb = interpreter.get_tensor(output_details[0]["index"])[0].astype(np.float32)  # [512]

    # normalize để cosine ổn định
...
def load_user_database() -> dict[str, np.ndarray]:
    """
    Đọc tất cả ảnh trong users/.
    - file name: "nam_1.jpg", "nam_2.jpg" -> user = "nam"
    - nếu nhiều ảnh/user -> lấy mean embedding rồi normalize lại
    """
...
def cosine_sim(a: np.ndarray, b: np.ndarray) -> float:
    # a,b đã L2 normalize -> dot chính là cosine
...
try:
    while True:
        raw = pipe.stdout.read(CAM_W * CAM_H * 3 // 2)
        if not raw:
            break
        ...
        if detection_results.detections and known_faces_db:
            ...
            for det in detection_results.detections:
                ...
                face_crop = safe_crop(frame_bgr, x, y, fw, fh)
                emb = get_face_embedding(face_crop)
                if emb is None:
                    continue
            ...
            if best_score >= COSINE_THRESHOLD:
                ...
                display_name = f"{best_name} ({best_score:.2f})"
            else:
                display_name = f"Unknown ({best_score:.2f})" if best_score > -1 else "Unknown"

            color = (0, 255, 0) if any_known_face_present else (0, 0, 255)
            if fw > 0 and fh > 0:
                cv2.rectangle(frame_bgr, (x, y), (x + fw, y + fh), color, 2)
                cv2.putText(frame_bgr, display_name, (x, max(20, y - 10)),
        ...
        if any_known_face_present:
            ...
            if elapsed >= REQUIRED_TIME:
                ...
                cv2.putText(frame_bgr, f"Hold still: {countdown:.1f}s", (x, y + fh + 30),
        ...
        cv2.imshow("Smart Door (FaceNet)", frame_bgr)
